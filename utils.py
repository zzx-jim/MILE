...
def simple_batch_tensor(features, lengths):
    node_features = []
    batch_size, dim = features.size(0), features.size(-1)
    for j in range(batch_size):
        feat = features[j, :lengths[j], :lengths[j], :]
...
def simple_batch_label(features, lengths):
    node_features = []
    batch_size = features.size(0)
    for j in range(batch_size):
        feat = features[j, :lengths[j], :lengths[j]]
